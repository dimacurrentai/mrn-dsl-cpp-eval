    ...
    for (auto const& [fiber_name, fiber] : maroon.fibers) {
      ...
      struct StatementsRecursiveVisitor final {
        ...
        void operator()(MaroonIRCode const& code) {
          fo << "    static void IMPL_" << (fiber_step_idx++) << kStepFunctionSignature << " {  // " << fn_name
             << std::endl
             << code.code << "" << std::endl
             << "}" << std::endl;
        }
        void operator()(MaroonIRBlock const&) {
          fo << "    static void IMPL_" << (fiber_step_idx++) << kStepFunctionSignature << " {  // " << fn_name
             << std::endl
             << "#error \"MaroonIRBLock is not yet implemented.\"" << std::endl
             << "}" << std::endl;
        }
      ...
      for (auto const& [fn_name, fn] : fiber.functions) {
        fo << "    inline constexpr static MaronStateIndex " << fn_name << " = static_cast<MaronStateIndex>("
           << visitor.fiber_step_idx << ");" << std::endl;
        ;
        visitor.fn_name = fn_name;
        fn.Call(visitor);
      }
      fo << "    inline constexpr static uint32_t kStepsCount = " << visitor.fiber_step_idx << ";" << std::endl;
      fo << "    inline constexpr static std::array<step_function_t, kStepsCount> kSteps{";
      for (uint32_t i = 0; i < visitor.fiber_step_idx; ++i) {
        if (i) {
          fo << ", ";
        }
        fo << "IMPL_" << i;

...
// TODO(dkorolev): This will be autogenerated soon.
CURRENT_VARIANT(CURRENT_MAROON_NAMESPACE_TYPES, MAROON_TYPE_U64, MAROON_TYPE_BOOL);
...
struct MaroonPackArgsImpl<std::tuple<OUT_TYPE, OUT_TYPES...>, IN_TYPE, IN_TYPES...> final {
  static void DoIt(std::vector<CURRENT_MAROON_NAMESPACE_TYPES>& res, IN_TYPE&& x, IN_TYPES&&... xs) {
...
struct MaroonPackArgsImpl<std::tuple<>> {
  static void DoIt(std::vector<CURRENT_MAROON_NAMESPACE_TYPES>&) {}
};

template <class OUT_TYPELIST, typename... IN_TYPES>
std::vector<CURRENT_MAROON_NAMESPACE_TYPES> pack_args(IN_TYPES&&... args) {
  std::vector<CURRENT_MAROON_NAMESPACE_TYPES> res;
...
struct ImplEnv final {
  ...
  // TODO(dkorolev): `init` should not be `uint64_t`.
  ...
  void DeclareVar(size_t idx, std::string name, uint64_t init) {
    ...
    var.value = CURRENT_MAROON_NAMESPACE_TYPES(std::move(val));
...
struct MaroonEngine final {
  // TODO(dkorolev): Types list specific to this `T_MAROON`.
  using T_VARS_TYPELIST = CURRENT_MAROON_NAMESPACE_TYPES;
